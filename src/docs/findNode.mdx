...
In a new Terminal (on MacOS/Linux) or Command Prompt (on Windows) window, send an [RPC request](https://www.google.com) to your node to retrieve its ID. This ID uniquely identifies your node in the network.
...
<TipsArea type="note" text="If you have changed the default RPC address `http://localhost:8114/` in your CKB [node configuration](https://www.google.com), make sure to replace it in the commands above." />
